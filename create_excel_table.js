...
// Daten aus der Datenbankabfrage (vereinfacht - hier nur Beispiele)
...
// HTML-Tabelle für Excel erstellen
...
<head>
    <meta charset="UTF-8">
    <title>GroupTypes Übersicht</title>
    <style>
        table {
        ...
        .kategorie { background-color: #e6f3ff; }
        .art { background-color: #f0f8ff; }
        .gruppierung { background-color: #f5f5f5; }
        .sparte { background-color: #fff8dc; }
        .form-name { background-color: #f0fff0; }
        .teil { background-color: #fff0f5; }
    </style>
</head>
<body>
    <h1>GroupTypes Übersicht - Alle Auswahlmöglichkeiten</h1>
    <table>
        <thead>
            <tr>
                <th class="kategorie">Kategorie</th>
                <th class="art">Art</th>
                <th class="gruppierung">Gruppierung</th>
                <th class="sparte">Sparte</th>
                <th class="form-name">Form_Name</th>
                <th class="teil">Teil 1 (deutsch)</th>
                <th class="teil">Teil 2 (deutsch)</th>
                <th class="teil">Teil 3 (deutsch)</th>
                <th class="teil">Teil 4 (deutsch)</th>
            </tr>
        </thead>
        <tbody>
`;
  ...
  groupTypesData.forEach(item => {
    ...
    html += `
            <tr>
                <td class="kategorie">${item.kategorie}</td>
                <td class="art">${item.art}</td>
                <td class="gruppierung">${item.gruppierung}</td>
                <td class="sparte">${item.sparte}</td>
                <td class="form-name">${item.form_name}</td>
                <td class="teil">${teil1}</td>
                <td class="teil">${teil2}</td>
                <td class="teil">${teil3}</td>
                <td class="teil">${teil4}</td>
            </tr>
`;
  });
  ...
  html += `
    ...
    <p><strong>Anzahl Einträge:</strong> ${groupTypesData.length}</p>
    <p><em>Hinweis: Diese Tabelle zeigt die Zuordnung zwischen Kategorien, Arten, Gruppierungen und Sparten sowie die entsprechenden Form-Namen mit deutschen Übersetzungen.</em></p>
</body>
</html>`;
...
fs.writeFileSync('group_types_table.html', htmlContent, 'utf8');

console.log('HTML-Tabelle wurde in group_types_table.html gespeichert');
console.log('Du kannst diese Datei in Excel öffnen:');
console.log('1. Excel öffnen');
console.log('2. Datei -> Öffnen');
console.log('3. group_types_table.html auswählen');
console.log('4. Als Webseite öffnen');
console.log('5. Dann als Excel-Datei speichern (Datei -> Speichern unter -> Excel-Arbeitsmappe)');

// Zusätzlich CSV erstellen für einfacheren Import
let csvContent = 'Kategorie,Art,Gruppierung,Sparte,Form_Name,Teil1_Deutsch,Teil2_Deutsch,Teil3_Deutsch,Teil4_Deutsch\n';
...
fs.writeFileSync('group_types_table.csv', csvContent, 'utf8');
console.log('CSV-Datei wurde in group_types_table.csv gespeichert');
console.log('Du kannst diese CSV-Datei direkt in Excel importieren');
