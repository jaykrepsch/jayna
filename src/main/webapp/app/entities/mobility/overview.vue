      ...
      <TableLayout>
        <template #head>
          <tr>
            ...
            <th scope="col" class="text-left px-3 py-3.5 text-sm font-semibold">
              {{ $t('jaynaApp.baseOverview.owner') }} <br>
              {{ $t('jaynaApp.baseOverview.user') }} <br>
              {{ $t('jaynaApp.baseOverview.licensePlate') }}
            </th>        
            <th scope="col" class="text-left px-3 py-3.5 text-sm font-semibold">
              {{ $t('jaynaApp.baseOverview.dateFirstRegistration') }} <br>
              {{ $t('jaynaApp.baseOverview.model') }} <br>
              {{ $t('jaynaApp.baseOverview.vin') }}
            </th>
            <th scope="col" class="text-left px-3 py-3.5 text-sm font-semibold">
              {{ $t('jaynaApp.baseOverview.purchase') }} <br>
              {{ $t('jaynaApp.baseOverview.purchasePrice') }} <br>
              {{ $t('jaynaApp.baseOverview.vehicleType') }}
            </th>
        ...
        <template #body>
          ...
          >
            ...
            <td class="px-3 py-4 text-sm">
              {{ subEntity.owner || '-' }} <br>
              {{ subEntity.user || '-' }} <br>
              {{ subEntity.licensePlate || '-' }}
            </td>
            <td class="px-3 py-4 text-sm">
              {{ subEntity.dateFirstRegistration || '-' }} <br>
              {{ subEntity.model || '-' }} <br>
              {{ subEntity.vin || '-' }}
            </td>
            <td class="px-3 py-4 text-sm">
              ...
              {{ subEntity.vehicleType ? $t('enums.' + subEntity.vehicleType) : '-' }}
            </td>
