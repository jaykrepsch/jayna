...
public class CategoryResource {
  ...
  /**
   * {@code GET  /categories/name/:name} : get the "name" category.
   *
  ...
  /**
   * {@code GET  /categories/entityName/:entityName} : get the "entityName"
   * category.
   *
   * @param entityName the entityName of the category to retrieve.
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
   *         the category, or with status {@code 404 (Not Found)}.
   */
  @GetMapping("/categories/entityName/{entityName}")
  public ResponseEntity<Category> getCategoryByEntityName(@PathVariable String entityName) {
    log.debug("REST request to get Category : {}", entityName);
    Optional<Category> category = categoryRepository.findOneByEntityName(entityName);
    return ResponseUtil.wrapOrNotFound(category);
  }
