...
class SubCategoryResourceIT {
  ...
  void createSubCategory() throws Exception {
    ...
    SubCategory testSubCategory = subCategoryList.get(
        subCategoryList.size() - 1);
    assertThat(testSubCategory.getName()).isEqualTo(DEFAULT_NAME);
    assertThat(testSubCategory.getCreatedDate())
        .isEqualTo(DEFAULT_CREATED_DATE);
    assertThat(testSubCategory.getTranslationKey())
  ...
  void getAllSubCategories() throws Exception {
    ...
    restSubCategoryMockMvc
        ...
        .andExpect(
            jsonPath("$.[*].id").value(hasItem(subCategory.getId().intValue())))
  ...
  void putNewSubCategory() throws Exception {
    ...
    SubCategory updatedSubCategory = subCategoryRepository
        .findById(subCategory.getId())
        .orElseThrow();
    // Disconnect from session so that the updates on updatedSubCategory are not
  ...
  void deleteSubCategory() throws Exception {
    ...
    restSubCategoryMockMvc
        .perform(
            delete(ENTITY_API_URL_ID, subCategory.getId())
