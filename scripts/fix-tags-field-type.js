...
function fixTagsField(filePath) {
  try {
    const content = fs.readFileSync(filePath, 'utf8');
    const formDef = JSON.parse(content);
    
    console.log(`🔧 Korrigiere tags-Feld: ${path.basename(filePath)}`);
    ...
    formDef.parts.forEach(part => {
      if (part.fields) {
        part.fields.forEach(field => {
          // Ändere tags von text zu textarea
          if (field.name === "tags" && field.type === "text") {
            field.type = "textarea";
            changes++;
          }
        });
...
function main() {
  console.log('🔧 Korrigiere tags-Feld in Dokument-Form-Definitionen...\n');
  ...
  files.forEach(file => {
    ...
    const changes = fixTagsField(filePath);
